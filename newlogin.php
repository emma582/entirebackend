...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($email_err) && empty($password_err)){
      //  $role=$_POST['role']; 
        ...
        $sql = "SELECT id,first_name,year,role,email_id, password FROM user_table WHERE email_id = ? AND password = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) > 0){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id,$firstname,$year, $role,$email, $password);
